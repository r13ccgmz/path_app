...
<head>
    <meta charset="utf-8">
    <title>Student History (with Grades) — {{ $studentInfo['name'] ?? 'Unknown Student' }}</title>
    <style>
        /* ── Reset & Base ──────────────────────────────── */
...
<body>
    ...
    <div class="header">
        ...
        <h2>Student Academic Record — PDF (with Grades)</h2>
        ...
        @foreach ($academicProgress as $progress)
            ...
            <div class="ao-entry" style="page-break-inside: avoid; margin-top: {{ $loop->first ? '0' : '16px' }}; {{ !$loop->last ? 'border-bottom: 1px solid #e2e8f0; padding-bottom: 12px;' : '' }}">
                <table class="info-grid" style="margin-bottom: 4px;">
                    <tr>
                        ...
                        <td style="width: 15%;">
                            ...
                            <div class="info-value" style="color: #166534;">{{ number_format($gwa, 4) }}</div>
                        </td>
                        @endif
                ...
                <div style="margin-top: 10px; border-top: 1px solid #e2e8f0; padding-top: 8px;">
                    <div class="info-label" style="font-size: 10px; margin-bottom: 6px;">Courses Taken</div>
                    <table class="data-table" style="margin-bottom: 0;">
                        ...
                        <tbody>
                            @foreach($progress['courses_taken_by_type'] as $typeLabel => $courses)
                                @php $flatCourses = isset($courses[0]) ? $courses : collect($courses)->flatten(1)->all(); @endphp
                                @foreach($flatCourses as $course)
                                <tr>
                                    <td><strong>{{ $course['course_code'] }}</strong></td>
                                    <td>{{ $course['course_name'] }}</td>
                                    <td><span style="font-size: 8px; font-weight: 700; text-transform: uppercase; color: {{ $course['type_color'] ?? '#475569' }};">{{ $course['type'] }}</span></td>
                                    <td class="text-center">{{ $course['units'] }}</td>
                                    <td class="text-center"><strong>{{ $course['grade'] ?? '-' }}</strong></td>
                                    <td>{{ $course['term_taken'] ?? '-' }}</td>
                                    <td class="text-center">
                                        @php
                                            $cStatus = strtolower($course['status'] ?? '');
                                            $cBadge = 'badge-gray';
                                            if ($cStatus === 'completed') $cBadge = 'badge-success';
                                            if ($cStatus === 'enrolled') $cBadge = 'badge-info';
                                            if ($cStatus === 'failed') $cBadge = 'badge-danger';
                                        @endphp
                                        <span class="badge {{ $cBadge }}">{{ ucfirst($cStatus) }}</span>
                                    </td>
                                </tr>
                                @endforeach
                            @endforeach
