    ...
    <x-filament::section class="fi-section-overview">
        ...
        <div class="px-6 py-4 border-b border-gray-200 dark:border-white/10 bg-gray-50/50 dark:bg-white/5 flex items-center justify-between">
            ...
            @if(!empty($filterFrom) || !empty($filterTo))
                @php
                    $fromLabel = $termsMap[$filterFrom] ?? $filterFrom;
                    $toLabel = $termsMap[$filterTo] ?? $filterTo;

                    if (!empty($filterFrom) && !empty($filterTo)) {
                        $filterSummary = $fromLabel . '  →  ' . $toLabel;
                    } elseif (!empty($filterTo)) {
                        $filterSummary = 'All terms up to ' . $toLabel;
                    } else {
                        $filterSummary = $fromLabel . ' onwards';
                    }
                @endphp
                <span class="text-xs font-semibold px-3 py-1 bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400 rounded-md">
                    {{ $filterSummary }}
                </span>
            @endif
        </div>
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
                ...
                <div class="rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 shadow-sm p-4 flex flex-col items-center text-center">
                    ...
                    <span class="text-[9px] text-gray-400 dark:text-gray-500 mt-0.5">Unique Students</span>
                </div>
                <div class="rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 shadow-sm p-4 flex flex-col items-center text-center">
            ...
            <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 rounded-xl p-5 shadow-sm" wire:key="overview-chart-{{ $filterFrom ?? 'all' }}-{{ $filterTo ?? 'all' }}">
                <div class="mb-4">
